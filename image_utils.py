...
def _get_s3_client():
    """Initialise the boto3 S3 client using project settings."""
    return boto3.client(
        "s3",
...
def process_profile_image(content: bytes) -> tuple[bytes, str]:
    """Process and resize the profile image in memory.

    Returns the processed image bytes and a unique filename.
    """
...
def _delete_from_s3(key: str) -> None:
    """Synchronous delete task from S3."""
    s3 = _get_s3_client()
    s3.delete_object(Bucket=settings.s3_bucket_name, Key=key)


async def upload_profile_image(file_bytes: bytes, filename: str) -> None:
    """Offloads the blocking S3 upload to a background thread."""
...
async def delete_profile_image(filename: str | None) -> None:
    """Offloads the blocking S3 deletion to a background thread."""
    if filename is None:
        return
    key = f"profile_pics/{filename}"
